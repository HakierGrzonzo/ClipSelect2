...
function addLeadingZero(n: number, fixedPrecision: number = 0): string {
  if (n === 0) {
    return '00'    
  }
  if (n < 10) {
...
const reducer = (state: range, action: number): range => {
  ...
  if (start > end) return [action, action]
  if (action < start) return [action, end]
  if (action > end) return [start, action]
  if (action <= mid) return [action + 1, end]
  if (action > mid) return [start, action - 1]
  return state
} 
